...
class SystemCameraTextureView1 : TextureView, TextureView.SurfaceTextureListener {

    private val TAG = SystemCameraTextureView1::class.java.simpleName
    ...
    private val mBitmap: Bitmap by lazy {
        Bitmap.createBitmap(
            Camera1Manager1.PREVIEW_WIDTH, Camera1Manager1.PREVIEW_HEIGHT,
    ...
    override fun onSurfaceTextureAvailable(surface: SurfaceTexture, width: Int, height: Int) {
        ...
        ret =
            CameraProcessLib.prepareSystemCamera(Camera1Manager1.RAW_PREVIEW_WIDTH, Camera1Manager1.RAW_PREVIEW_HEIGHT)
    ...
    private fun processSystemCameraJob(): Job = GlobalScope.launch {
        // 启动一个新协程并保持对这个作业的引用
        while (isActive) {
            val frame = Camera1Manager1.preBuffer
            CameraProcessLib.processSystemCamera1(frame)
            CameraProcessLib.pixelToBmp1(mBitmap)
            drawBitmap(mBitmap)
            delay(5L)
        }
    }
